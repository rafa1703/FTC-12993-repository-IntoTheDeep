...
import static org.firstinspires.ftc.teamcode.gvf.utils.DashboardUtil.drawRobot;
...
import static org.firstinspires.ftc.teamcode.system.hardware.IntakeSubsystem.slideTeleSubClose;
import static org.firstinspires.ftc.teamcode.system.hardware.IntakeSubsystem.slideTeleSubFar;
import static org.firstinspires.ftc.teamcode.system.hardware.IntakeSubsystem.slideTeleSubMid;
...
public class  SentinelDrive extends LinearOpMode
{
    ...
    double globalTimer, sequenceTimer, intakeClipTimer, turretTimer, internalTimer, intakeArmTimer, climbWindUpTimer;
    ...
    boolean cachedIntakeSlidePos;
    boolean comingFromSpecDeposit = false;
    boolean hpDepositSlidesReturned;
    boolean startIntaking;
    boolean climbWindUp = false; // if the climb has started to wind up
    boolean comingFromSpecIntake;
...
//    ToggleUpOrDown liftFineAdjustBtn = new ToggleUpOrDown(1, 1, 0);
    ...
    ToggleUpOrDownCircular togglePivot = new ToggleUpOrDownCircular(1, 1, 4, 7);
    ...
    ToggleRisingEdge toggleArmAlwaysUp  = new ToggleRisingEdge();
    ToggleRisingEdge toggleClimbWindUp  = new ToggleRisingEdge();
    ...
    double slidePositionStopped;
    boolean autoSampleOuttake = true; // this should start as true ig
    boolean intaked;
    boolean intakingFromBars;
    boolean autoAlignSpecDeposit = false;
    boolean autoAlignSampleDeposit = false; // this starts off
    boolean autoAlignHpDeposit = false;
    boolean autoAlignSpecIntake = false;
    boolean autoAlignSpecDepositBack = true; // auto align here does not work, too much slack on the outtake
    ...
    boolean comingFromSampleDeposit;
    boolean armAlwaysUp;
    boolean climbActuated;
...
//            else gamepad2.setLedColor(255, 0, 0, 1000);
        ...
        if (opModeIsActive())
        {
            hardware = new GeneralHardware(hardwareMap, side, true);
            hardware.limelight.shutdown();
...
//            outtakeSubsystem.turretSpinTo(180);
        ...
        while (opModeIsActive())
        {
            ...
            if (state != OuttakeState.SPECIMEN_DEPOSIT_BACK_AUTO &&
                ...
                driveBase.drive(gamepad1.left_stick_y, gamepad1.left_stick_x, gamepad1.right_stick_x,
                        state == OuttakeState.INTAKE_EXTENDO_SUB ||
                                ...
                                (state == OuttakeState.SPECIMEN_DEPOSIT && startIntaking)
                );
...
//            telemetry.addData("Climb timer",  climbWindUpTimer);
            ...
            telemetry.addData("Intake pos", intakeSubsystem.getSlidePositionIn());
...
//            telemetry.addData("Toggle auto allign spec dep", autoAlignSpecDeposit);
            telemetry.addLine();
            ...
            telemetry.addData("SPEC STATE", goToHPExtendoDeposit);
//            telemetry.addData("FL", hardware.FL.getCurrent(CurrentUnit.AMPS));
//            telemetry.addData("FR", hardware.FR.getCurrent(CurrentUnit.AMPS));
//            telemetry.addData("BL", hardware.BL.getCurrent(CurrentUnit.AMPS));
//            telemetry.addData("BR", hardware.BR.getCurrent(CurrentUnit.AMPS));
...
//            telemetry.addDa2ta("Angular velocity", angularVel);
//
//            telemetry.addData("FilterState", intakeSubsystem.intakeFilter);
//            telemetry.addData("Color value", intakeSubsystem.getColorValue());
//            telemetry.addData("ColorSensor distance", intakeSubsystem.getColourSensorDistance());
        ...
        switch (state)
        {
            case READY:
                if (gamepad1.right_bumper)
                {
                    ...
                    resetTimer();
                    break;
                } else if (gamepad1RightTrigger() && false)
                {
//                    state = OuttakeState.INTAKE_EXTENDO;
                ...
                } else if (gamepad1.left_bumper)
                {
                    ...
                    intakeSlideTarget = slideTeleSubFar;
                    ...
                    toggleRisingEdge.mode(gamepad1.left_bumper); // new control way
                ...
                if (gamepad2LeftTrigger())
                {
                    state = OuttakeState.SPECIMEN_INTAKE;
                    toggleRisingEdge.mode(gamepad1.right_bumper);
                    comingFromSpecDeposit = false;
                    comingFromSpecIntake = true;
                    intaked = false;
                    resetTimer();
                    break;
                }
                outtakeSubsystem.armState(OuttakeSubsystem.OuttakeArmServoState.TRANSFER_BACK); // necessary because of the really high delays in return
                ...
                break;
//            case INTAKE_EXTENDO: // this state is just for using the extendo outside of the submersible, i should remove this state as sub one is auto
//                intakeSlideBtn.upToggle(gamepad1RightTrigger());
...
//                intakeSlideBtn.upToggle(gamepad1LeftTrigger());
//                intakeSlideBtn.downToggle(gamepad1.left_bumper, 1);
                if (intakeSlideBtn.OffsetTargetPosition == 1) intakeSlideTarget = slideTeleFar;
//                if (intakeSlideBtn.OffsetTargetPosition == 2) intakeSlideTarget = slideTeleClose;
                ...
                if (delay(50))
                {
                    ...
                    if (delay(120)) // waits until the arm is out
                    {
                        intakeTurretPresetsOrNot(gamepad1.right_bumper, gamepad1.left_bumper, false);
                    }
                    outtakeSubsystem.armState(OuttakeSubsystem.OuttakeArmServoState.TRANSFER_BACK);
                    outtakeSubsystem.wristState(OuttakeSubsystem.OuttakeWristServoState.TRANSFER_BACK);
                    outtakeSubsystem.pivotServoState(OuttakeSubsystem.OuttakePivotServoState.DOWN);
                    outtakeSubsystem.clawSetPos(0.92);
                    if (delay(500)) outtakeKeepTurretBack(); // dont knock specs of lmao
                    if ((delay(700) && sampleInIntake()) ||
                            gamepad1.left_stick_button)
                    {
                        state = OuttakeState.AFTER_EXTENDO;
                        intakeSubsystem.intakeClip(IntakeSubsystem.IntakeClipServoState.OPEN);
            ...
            case INTAKE_EXTENDO_SUB: // this is the worst ftc game ever created
                // the intake part
                // this has to be outside, so i can double click it
//                intakeSlideSubOffset();

                colourValue = intakeSubsystem.getColourValue();
                intakeSubsystem.intakeClip(IntakeSubsystem.IntakeClipServoState.OPEN);
                //outtakeSubsystem.armState(OuttakeSubsystem.OuttakeArmServoState.STRAIGHT);

                if (outtakeSubsystem.getPositionIn() < 4) {
                    outtakeSubsystem.liftToInternalPID(0);
                    outtakeSubsystem.armState(OuttakeSubsystem.OuttakeArmServoState.TRANSFER_BACK);
                } else if (delay(50)) outtakeSubsystem.liftMotorRawControl(-1);
                ...
                if (delay(110))
                {
//                    fineAdjustingOrNotIntakeSlides();
                    if (delay(750))
                        outtakeKeepTurretBack();
                    hpExtendoToggle();
                    autoSampleOuttakeToggle(gamepad1.a);
                    extendoReturnFromBarsToggle(gamepad1.y || gamepad2.dpad_up);
                    ...
                    if (!intakeEdgeCase)
                    {
                        if (internalDelay(250)) intakeTurretPresetsOrNot(gamepad1RightTrigger(), gamepad1LeftTrigger(), armWentDown);
                        if (armWentDown)
                        {
                            if (gamepad1.right_bumper) intakeSubsystem.intakeArm(IntakeSubsystem.IntakeArmServoState.HALF_DOWN); // if i fuck up the timing
                            else intakeSubsystem.intakeArm(IntakeSubsystem.IntakeArmServoState.EXTENDO_DOWN);

                            if ( globalTimer - intakeArmTimer > 350)
                            {
                                if (!cachedIntakeSlidePos) // runs once after timer is > 350ms
                                {
                                    slidePositionStopped = intakeSubsystem.getSlidePositionIn();
                                    cachedIntakeSlidePos = true;
                                }
                        ...
                        {
                            intakeSubsystem.intakeSlideInternalPID(comingFromSampleDeposit ? 13 : slideExtensionLimit);
                            if (comingFromSampleDeposit && gamepad1.left_bumper)
                            {
                                toggleRisingEdge.mode(gamepad1.left_bumper);
                                comingFromSampleDeposit = false; // so this will make it extend again...
                            }
                            if (toggleRisingEdge.mode(gamepad1.left_bumper) && !comingFromSampleDeposit)
                            {
                                intakeSubsystem.intakeArm(IntakeSubsystem.IntakeArmServoState.EXTENDO_DOWN);
                                armWentDown = true;
                                cachedIntakeSlidePos = false;
                                intakeArmTimer = globalTimer;
                            }
                        }
                    }
                    else
                    {
                        intakeSubsystem.intakeSlideMotorRawControl(-gamepad2.left_stick_y);
                    ...
                    if (gamepad1.right_stick_button)
                        intakeSubsystem.intakeSpin(IntakeSubsystem.IntakeSpinState.REVERSE);
                    else if (armWentDown) //if (intakeArmToggle.OffsetTargetPosition > 1)
                        intakeSubsystem.intakeSpin(IntakeSubsystem.IntakeSpinState.INTAKE);

                    if (
                            ((delay(300) && (sampleInIntake() && cachedIntakeSlidePos && globalTimer - intakeArmTimer > 500))
            ...
            case AFTER_EXTENDO: // this might be necessary to check the colour and go back the slides
                if (((
                        (!metaTransferEdgeCase ? delay(200) : delay(700)) && intakeSubsystem.isSlidesAtBase())
                        || (!metaTransferEdgeCase ? delay(900) : delay(1000))
                )
                        && !gamepad1.right_stick_button)
                { // checks hpDepExtendo -> edgeCases -> intakingFromBars

                    state = goToHPExtendoDeposit && isSample ? OuttakeState.HP_DEPOSIT_EXTENDO :
                            extendoReturnFromBars || auraTransferEdgeCase ? OuttakeState.TRANSFER_START :
                    ...
                    if (goToHPExtendoDeposit) intakeSlideTarget = 0;
                    toggleRisingEdge.mode(gamepad1.right_bumper);
                    resetTimer();
                    break;
                }
                hpExtendoToggle();
                if (!intakeEdgeCase && !extendoReturnFromBars && !goToHPExtendoDeposit && metaTransferEdgeCase && delay(50)) // basically meta transfer has the least priority
                    outtakeSubsystem.turretSpinToGains(OuttakeSubsystem.OuttakeTurretState.TRANSFER_META);
                else outtakeKeepTurretBack();
                autoSampleOuttakeToggle(gamepad1.a);
                extendoReturnFromBarsToggle(gamepad1.left_bumper || gamepad2.y);
                auraTransferToggle(gamepad2.dpad_right);
                metaTransferToggle(gamepad2.dpad_left);

                outtakeSubsystem.liftMotorRawControl(0); // this might be necessary
                if (gamepad1.right_stick_button)
                {
                ...
                else
                {
                    ...
                    if (intakeEdgeCase) // this case we want to make sure the arm is up before turning the turret
                    {
                        ...
                        if (delay(300))
                        {
                            intakeSubsystem.intakeTurret(IntakeSubsystem.IntakeTurretServoState.TRANSFER); //was straight
                        }
                        if (delay(600)) intakeSubsystem.intakeSlideMotorRawControl(-1); //intakeClipHoldLogic(-10, 5);
                        else intakeSubsystem.intakeSlideInternalPID(intakeSlideTarget + 3); // we need to go a little bit forward to clear the battery
                    }
                    else
                    {
                        if (extendoReturnFromBars)
                        {
                            ...
                            {
                                if (intakeSubsystem.getSlidePositionIn() < 2) intakeSubsystem.intakeSlideInternalPID(-1);
                        ...
                        else
                        {
                            ...
                            {
                                intakeSubsystem.intakeTurret(IntakeSubsystem.IntakeTurretServoState.TRANSFER);
                                intakeSubsystem.intakeArm(IntakeSubsystem.IntakeArmServoState.TRANSFER_BACK);
                            }
                            else if (!goToHPExtendoDeposit) // we dont wanna set up for edge case transfers if
                            {
                                outtakeSubsystem.turretSpinToGains(OuttakeSubsystem.OuttakeTurretState.TRANSFER_META);
                                ...
                                if (delay(350)) intakeSubsystem.intakeTurret(IntakeSubsystem.IntakeTurretServoState.TRANSFER_META);
                            }
//                            if (intakeSubsystem.ticksToInchesSlidesMotor(intakeSubsystem.slidePosition) < -4)
//                            {
//                                intakeSubsystem.intakeSlideInternalPID(-4);
//                            } else
                            if (intakeSubsystem.getSlidePositionIn() < 2) intakeSubsystem.intakeSlideInternalPID(-1);
                            else intakeSubsystem.intakeSlideMotorRawControl(-1);
                        }
            ...
            case INTAKE:
                ...
                if (!intakeEdgeCase)
                {
                    ...
                    if (delay(120)) // waits until the arm is out
                    {
                        intakeTurretPresetsOrNot(gamepad1RightTrigger(), gamepad1LeftTrigger(), true);
                    }
                ...
                else
                {
                    if (internalDelay(50))
                    {
                        if (internalDelay(550)) intakeSlideTarget = 11;
                        else intakeSlideTarget = 15;
                ...
                colourValue = intakeSubsystem.getColourValue();
                if (
                        ((delay(200) && !intakeEdgeCase) ||
                                (internalDelay(1000) && intakeEdgeCase)) &&
                        (gamepad1.left_stick_button)
                )
                {
            ...
            case TRANSFER_START: // this state is skipped when coming from the extendo and not from the bars
                if ((
                        ...
                        intakeSubsystem.isSlidesAtBase()) || (delay(600)))
                )
                {
                ...
                if (delay(90)) intakeSubsystem.intakeSlideMotorRawControl(0);
                else intakeSubsystem.intakeClip(IntakeSubsystem.IntakeClipServoState.HOLD);
...
//                        outtakeSubsystem.pivotServoState(OuttakeSubsystem.OuttakePivotServoState.RIGHT);
            ...
            case TRANSFER_END:
                ...
                if (delay(200))
                {
                    ...
                    {
                        state = false ? OuttakeState.SPECIMEN_DEPOSIT_BACK_AUTO : OuttakeState.SPECIMEN_DEPOSIT_BACK;
                        dropped = false;
                        intaked = false;
...
//                        }
                    ...
                    else
                    {
                        ...
                        if (!comingFromSpecDeposit) {
                            ...
                            outtakeSubsystem.clawState(OuttakeSubsystem.OuttakeClawServoState.CLOSE);
//                            throw new RuntimeException("Why is the wrist not moving broski");
                        }
                    }
            ...
            case HP_DEPOSIT:
                ...
                if (!dropped)
                {
                    if (delay(50))
                    {
                        ...
                        if (intakeEdgeCase) // we need to fix the arm
                        {   if (delay(150))
                                intakeSubsystem.intakeArm(IntakeSubsystem.IntakeArmServoState.HALF_TRANSFER);
                            if (delay(300)) {
                                intakeSubsystem.intakeSlideMotorRawControl(0);
                            ...
                            else intakeSubsystem.intakeSlideMotorRawControl(0.45);
                        }
                        else if (!hpDepositSlidesReturned)
                        {
                            hpDepositSlidesReturned = intakeSubsystem.isSlidesAtBase() && delay(300) || delay(1200);
                            internalTimerReset();
                        }
                        if (hpDepositSlidesReturned)
                        {
                            intakeSubsystem.intakeArm(IntakeSubsystem.IntakeArmServoState.HP_DEPOSIT);
                            if (internalDelay(300)) intakeSubsystem.intakeTurret(IntakeSubsystem.IntakeTurretServoState.HP_DEPOSIT);
                        }
                        else
                        {
                            intakeSubsystem.intakeTurret(IntakeSubsystem.IntakeTurretServoState.STRAIGHT);
                        }
                        if (!intakeEdgeCase || delay(300))
            ...
            case HP_DEPOSIT_EXTENDO:
                if (delay(200))
                {
                    ...
                    } else
                    {
                        ...
                        intakeSlideTarget = slideExtensionLimit;
                ...
                if (
                        ((internalDelay(200) && intakeSubsystem.getColourValue() < 150)
                        || internalDelay(300)) && dropped)
            ...
            case SAMPLE_DEPOSIT: // this will actually start the deposit, so lift and arm presets
                if (delay(750) && dropped) //secondToggleForTheDrop.mode(gamepad1.right_bumper) && delay(400) && dropped)
                {
                    state = OuttakeState.INTAKE_EXTENDO_SUB;
                    intakeSubsystem.intakeSlideMotorEncoderReset();
                    variablesResets();
                    comingFromSampleDeposit = true;
                    resetTimer();
                    break;
                }
                liftHeightLogic(gamepad2.x, gamepad2.a);
                ...
                if (!dropped)
                {
                    if (delay(40))
                    {
                        if (metaTransferEdgeCase)
                            intakeSubsystem.intakeTurret(IntakeSubsystem.IntakeTurretServoState.STRAIGHT);
                        if (gamepad2.a || gamepad2.x) fineAdjustingLiftSample = false;
                        else if (Math.abs(gamepad2.right_stick_y) > 0) fineAdjustingLiftSample = true;
                        ...
                        {
                            outtakeLiftPresets(metaTransferEdgeCase); // this just runs the correct height for the lift
                        }

                        toggleAutoAlign();
                        if (!isSampleLow ? outtakeSubsystem.liftPosition > outtakeSubsystem.liftTarget * 0.63 || delay(30) :
                        delay(100))
                        {
                            if (!metaTransferEdgeCase)
                            {
                            ...
                            {
                                outtakeSubsystem.armSetPos(0.45);
                                outtakeSubsystem.wristSetPos(0.55);
                            }
                        }
                        ...
                        if (delay(200))
                        {
                            if (metaTransferEdgeCase) outtakeSubsystem.pivotServoState(OuttakeSubsystem.OuttakePivotServoState.RIGHT_DOWN);
                            else if (!gamepad2.dpad_down) outtakeSubsystem.pivotSetPosByOrder(togglePivot.OffsetTargetPosition);
                            else togglePivot.OffsetTargetPosition = 0;
                        }
                ...
                {
//                    outtakeSubsystem.wristSetPos(0.55);
                    if (delay(200) && !metaTransferEdgeCase)
                    {
//                        outtakeSubsystem.wristState(OuttakeSubsystem.OuttakeWristServoState.SAMPLE_DROP);
                        if (!isSampleLow) liftToMaxHeight(); // our climb hooks get caught on the low bucket if we lift more just wrist
                    }
                }
                break;
            case SPECIMEN_DEPOSIT: // we want to deposit and intake
                ...
                switch (specDepositState) // lol i should have done this to like all the code lmao
                {
                    case DEPOSIT:
                        ...
                        if (delay(40))
                        {
                            if (autoAlignSpecDeposit) // this should have definitely have been a function lol
                            {
                                if (isBetweenAngle(heading, -40,30)) //heading <= 45 && heading >= -45),
                                {
                                    outtakeSubsystem.turretKeepToAngleTicks(0, heading);
                                } else outtakeSubsystem.turretRawControl(0);
                            }
                            ...
                            outtakeSubsystem.liftToInternalPID(6); // 14in
...
//                            outtakeSubsystem.wristSetPos(0.45);
//                            outtakeSubsystem.armSetPos(0.75);
                            ...
                            if (toggleRisingEdge.mode(gamepad1.right_bumper))
                            {
                                ...
                                outtakeSubsystem.clawState(OuttakeSubsystem.OuttakeClawServoState.OPEN); // the real drop actually only happens here lol
                                internalTimerReset();
                                break;
                            }
                        }
                    ...
                    case DROP: // we want to be able to drag in this
                        ...
                        {
                            if (isBetweenAngle(Angles.normalizeDegrees(heading), -40, 30)) //heading <= 45 && heading >= -45),
                            {
                                outtakeSubsystem.turretKeepToAngleTicks(0, heading);
                            } else outtakeSubsystem.turretRawControl(0);
                        }
                    ...
                    case RETURN:
                        ...
                        if (toggleRisingEdge.mode(gamepad1.left_bumper) && !startIntaking) {
                            secondToggleForTheDrop.mode(gamepad1.left_bumper);
                            startIntaking = true;
                        }
                        if (!startIntaking) intakeTurretPresetsOrNot(gamepad1RightTrigger(), gamepad1LeftTrigger(), false);
                        if (internalDelay(150))
                        {
                            outtakeSubsystem.liftToInternalPID(0);
                            if (internalDelay(400)) {
                        ...
                        if (internalDelay(500)) outtakeSubsystem.turretSpinTo(OuttakeSubsystem.OuttakeTurretState.TRANSFER_FRONT);
                        else outtakeSubsystem.turretRawControl(0);
                        break;
                }

                // the intaking part
                if (delay(110) && startIntaking)
                {
                    if (toggleIntakeEdgeCase.mode(gamepad2.dpad_down || gamepad1.dpad_down))
                    {
                        intakeEdgeCase = !intakeEdgeCase;
                    ...
                    if (!intakeEdgeCase)
                    {
                        ...
                        {
                            intakeSubsystem.intakeSlideInternalPID(slideExtensionLimit, 0.85);
                            if (secondToggleForTheDrop.mode(gamepad1.left_bumper))
                            {
                                intakeSubsystem.intakeArm(IntakeSubsystem.IntakeArmServoState.EXTENDO_DOWN);
                                armWentDown = true;
                                cachedIntakeSlidePos = false;
...
//                toggleAutoAlign();
                ...
                {
                    double depHeight = outtakeSubsystem.liftHighBarBackStaticPos + 9;
                    outtakeSubsystem.liftToInternalPID(depHeight);
                ...
                if (delay(40))
                {
                    if (!dropped)
                    {
                        if(hardware.drive.getPredictedPoseEstimate().getY() > -37.2 && !gamepad1.right_stick_button)
                        {
                            ...
                            hardware.drive.updateDrive();
                            xPosition += -gamepad1.left_stick_y * 0.5;
                            telemetry.addData("Auto drive: ", true);
                        }
                        else
                        {
                        ...
                        else intakeSubsystem.intakeArm(IntakeSubsystem.IntakeArmServoState.HORIZONTAL); // low key just try to not get stuck in the spec
                        if (toggleRisingEdge.mode(gamepad1.right_bumper))
                        {
                            dropped = true;
                            secondToggleForTheDrop.mode(gamepad1.right_bumper);
...
//                        if (internalDelay(450)) hardware.drive.setTargetPose(new Pose(xPosition - 8, -38, Math.toRadians(-18)));
                        ...
                        if (secondToggleForTheDrop.mode(gamepad1.right_bumper) && internalDelay(890))
                        {
                            state = OuttakeState.INTAKE_EXTENDO_SPEC;
                            intakeSlideTarget = slideTeleFar;
                            intakeSubsystem.intakeClip(IntakeSubsystem.IntakeClipServoState.OPEN);
                            variablesResets(); // something was causing a bug when swapping this states but it worked if i presed b
                            resetTimer();
                            break;
                        }
                    }
                ...
                break;
            case HANG_START:
                if (gamepad1.right_bumper && delay(300))
                {
                    state = OuttakeState.HANG_LEVEL2;
                    toggleRisingEdge.mode(gamepad1.right_bumper);
                    outtakeSubsystem.liftMotorRawControl(0);
...
//                outtakeSubsystem.turretSpinTo(OuttakeSubsystem.OuttakeTurretState.TRANSFER_BACK);
                if (delay(1450)) driveBase.climbServoSetPower(-0.15); // this should hold
                ...
                if (delay(400))
                {
                    driveBase.ptoMotorsSetPower(0.25);
                }
                break;
            case HANG_LEVEL3:
                ...
                double climbPow = 0.97;
                intakeSubsystem.intakeSlideInternalPID(0);
                outtakeSubsystem.liftMotorRawControl(-climbPow);
                driveBase.ptoMotorsSetPower(climbPow);
            ...
            case HANG_END:
                ...
                boolean turned = delay(2000) && outtakeSubsystem.turretIncrementalPosition < 500;
                if (delay(500))
                {
                ...
                if (turned)
                {
                    ...
                    outtakeSubsystem.armSetPos(0.9);
                }
                else intakeSubsystem.intakeSlideInternalPID(0);
                break;
            case RETURN:
                if ((outtakeSubsystem.liftAtBase() && intakeSubsystem.isSlidesAtBase() && delay(500))
                        || delay(1350))
                {
                    variablesResets();
                    ...
                    outtakeSubsystem.liftMotorRawControl(0);
                    intakeSubsystem.intakeSlideMotorRawControl(0);
                    intakeSubsystem.intakeSlideMotorEncoderReset(); // this is
                ...
                if (outtakeSubsystem.getPositionIn() < 4) outtakeSubsystem.liftToInternalPID(-1);
                else outtakeSubsystem.liftMotorRawControl(-1);

                outtakeSubsystem.clawState(OuttakeSubsystem.OuttakeClawServoState.CLOSE);
                if (!comingFromSpecIntake || delay(470))
                    outtakeSubsystem.armState(OuttakeSubsystem.OuttakeArmServoState.TRANSFER_BACK);

                if (intakeSubsystem.isSlidesAtBase()) // slides are never at base ate the start of edge case
                {
                    intakeSubsystem.intakeArm(IntakeSubsystem.IntakeArmServoState.READY);
                }
                ...
                {
                    outtakeSubsystem.pivotServoState(OuttakeSubsystem.OuttakePivotServoState.DOWN);
                    if ((!comingFromSpecIntake && !comingFromSpecDeposit) || delay(450))
                        outtakeSubsystem.wristState(OuttakeSubsystem.OuttakeWristServoState.TRANSFER_BACK);
                    outtakeKeepTurretBack();
                    if (intakeEdgeCase)
                    {
                ...
                else if (comingFromSpecDeposit || comingFromSpecIntake) outtakeSubsystem.wristSetPos(1);
                break;
        ...
        if ((
                (gamepad1.b || (gamepad2.b && !gamepad2.options && !gamepad1.options)) && // not gamepad1.options if i want override her controller
                (state != OuttakeState.READY) &&
        ...
        if ( // lol or else climb servo is set to 0
                state != OuttakeState.HANG_LEVEL2 &&
                state != OuttakeState.HANG_LEVEL3 &&
                state != OuttakeState.HANG_END
        )
        {
            if (toggleClimbWindUp.mode(gamepad1.touchpad)) { // once i press this is pressed forever lol
                if (!climbWindUp) {
                    climbWindUp = true;
                    climbWindUpTimer = globalTimer;
                    driveBase.climbServoSetPower(0); // lol bug
                }
                if (climbActuated) {
        ...
        if (gamepad1.dpad_up)
        {
            hardware.reInitImu(); // this method already resets the yaw
//            imu.resetYaw();
        }
    ...
    public void climbWindUpAsync()
    {
        if (climbWindUp) { // this checks if we pressed the wind up button
            if (climbActuated) { // stops the servo after time elapsed
                driveBase.climbServoSetPower(0);
            } else if (globalTimer - climbWindUpTimer <= 4300) driveBase.climbServoSetPower(1);
            else climbActuated = true;
        }
    }
    public void variablesResets()
    {
        // variable resets
        isSample = true;
        goToIntake = false;
        ...
        goToHPDeposit = false;
//                    goToHPExtendoDeposit = false; // going to test not reseting this
        dropped = false;
        intaked = false;
//                    intakeTurretUsingPresets = false; // removed this so i only have to toggle it off once
        ...
        comingFromSampleDeposit = false;
        comingFromSpecDeposit = false;
        cachedIntakeSlidePos = false;
        startIntaking = false;
        comingFromSpecIntake = false;
        ...
        intakeArmToggle.OffsetTargetPosition = 0;
        intakeTurretToggle.OffsetTargetPosition = 2;
        togglePivot.OffsetTargetPosition = 6; // 4 is vertical drop, 6 is horizontal
    ...
    public void intakeSlideSubOffset()
    {
        intakeSlideSubBtn.upToggle(gamepad1.right_bumper);
        intakeSlideSubBtn.downToggle(gamepad1.left_bumper, 1);
        if (intakeSlideSubBtn.OffsetTargetPosition == 1) intakeSlideTarget = slideTeleSubClose;
        if (intakeSlideSubBtn.OffsetTargetPosition == 2) intakeSlideTarget = slideTeleSubMid;
        if (intakeSlideSubBtn.OffsetTargetPosition == 3) intakeSlideTarget = slideTeleSubFar;
    }
    public void tinkoIntaking()
    {
        ...
        if (!armAlwaysUp) {
            ...
            if (toggleRisingEdgeD2Intake.mode(gamepad2.left_bumper || gamepad2.right_bumper)) {
                ...
                intakeSubsystem.intakeSlideMotorRawControl(-gamepad2.left_stick_y * 0.6);
            } else if (delay(110)) {
                intakeSubsystem.intakeSlideInternalPID(intakeSlideTarget);
            }
            colourValue = intakeSubsystem.getColourValue();
            ...
            if (delay(110)) {
                ...
                if (!intakeEdgeCase) {
                    if (internalDelay(300)) {
                        ...
                        intakeTurretPresetsOrNot(gamepad2RightTrigger(), gamepad2LeftTrigger(), false);
                    }
                } else {
                    if (internalDelay(300)) {
                        intakeSubsystem.intakeArm(IntakeSubsystem.IntakeArmServoState.EXTENDO_DOWN);
...
//                    break;
        ...
        else // if we are just cycling without simu intake
        {
    ...
    public void fineAdjustingOrNotIntakeSlides()
    {
        if (Math.abs(gamepad2.left_stick_y) > 0 && delay(1000)) // i swear i'm gonna remove this shit from her
        {
        ...
        if (!fineAdjustingIntakeSlides)
            intakeSubsystem.intakeSlideInternalPID(intakeSlideTarget);
        else intakeSubsystem.intakeSlideMotorRawControl(-gamepad2.left_stick_y * 0.6);
    }
    ...
    {
            if (toggle) metaTransferEdgeCase = true;
            else  metaTransferEdgeCase = false;
    }

    public void hpExtendoToggle()
    {
        if (hpExtendoToggle.mode(gamepad2.left_stick_button))
        {
            goToHPExtendoDeposit = !goToHPExtendoDeposit;
            gamepad1.rumbleBlips(1);
            gamepad2.rumbleBlips(1); // fufcking necessary
        }
    }
    public void armAlwaysUpToggle(boolean toggle)
    {
        if (toggleArmAlwaysUp.mode(toggle))
        {
            armAlwaysUp = !armAlwaysUp;
        }
    }
    public void toggleAutoAlign()
    {
        if (autoAlignToggle.mode(gamepad1.x)) // this is such a simplified way of doing this lol
        {
            ...
            else if (state == OuttakeState.SPECIMEN_DEPOSIT_BACK || state == OuttakeState.INTAKE_EXTENDO_SPEC)
            {
                autoAlignSpecDepositBack = !autoAlignSpecDepositBack;
            }
        }
    }
    public void updateHeading(boolean states)
    {
        if (states)
        {
    ...
    public void intakeTurretPresetsOrNot(boolean rightToggle, boolean leftToggle, boolean armWentDown)
    {
        intakeTurretToggle.upToggle(rightToggle);
        intakeTurretToggle.downToggle(leftToggle, 1);
        ...
        else if (armWentDown) intakeSubsystem.intakeTurretBasedOnHeadingVel(angularVel);
    }
    ...
    private void updatePose(boolean state)
    {
        if (state)
        {
    ...
    {
        wasPoseReset = true;
        hardware.drive.getLocalizer().resetPoseAndHeading();
        hardware.drive.getLocalizer().setOffSet(resetPoseOffsets);
        pose = hardware.drive.getLocalizer().getPoseEstimate();
    }
...
//        }
//    }
    public void outtakeLiftPresets(boolean metaTransferEdgeCase)
    {
        if (isSample)
        {
            if (isSampleLow)
            ...
            {
//
                liftToSampleMaxHeight(metaTransferEdgeCase);
            }
        }
        else
        {
    ...
    public void liftToSampleMaxHeight(boolean metaTransferEdgeCase)// WAY FASTER THAN INTERNAL PID
    {
        double sampleHeight =!metaTransferEdgeCase ? 33 : 39.9;
        if (outtakeSubsystem.ticksToInchesSlidesMotor(outtakeSubsystem.liftPosition) > sampleHeight - 1)
            outtakeSubsystem.liftToInternalPID(sampleHeight); // max heigh 26.1
        else outtakeSubsystem.liftMotorRawControl(0.99);
    }
    public void liftToMaxHeight()// WAY FASTER THAN INTERNAL PID
    {
        if (outtakeSubsystem.ticksToInchesSlidesMotor(outtakeSubsystem.liftPosition) > 32.2)
            outtakeSubsystem.liftToInternalPID(33.2); // max heigh 26.1
        else outtakeSubsystem.liftMotorRawControl(1);
    }
...
//    }
    ...
    {
        return intakeSubsystem.sampleInIntakeWithColourCheck(goToHPExtendoDeposit) && false; // we want to go to hp when cycling colour lmao
    }
